...
class FilteredWrappedCursor private constructor(
    ...
    private val mappedPath: SparseArray<String>,
    ...
    override fun getCount(): Int = mappedPath.size

    override fun getString(index: Int): String? = if (index == pathIndex) {
        val new = mappedPath[originCursor.position]
    ...
    override fun moveToLast(): Boolean = realMove(mappedPath.size - 1)
    ...
    private fun realMove(newPosition: Int): Boolean {
        ...
        if (originCursor.moveToPosition(mappedPath.keyAt(newPosition))) {
            mPos = newPosition
            return true
        } else {
            originCursor.moveToPosition(-1)
